$(document).ready(function() {
  ...
  $('.openFormAdd').click(function() {
		modalState = 'add';
    $('.modal-title').text('Tambah Barang Kandang');
  ...
  $('.openFormUpload').click(function() {
		$('#modal-upload-kandang .modal-title').text('Upload Barang Kandang Sekaligus');
		$('#modal-upload-kandang').modal('show');
		$('.validate-error').html('');
	});
  ...
  $('.downloadExcel').click(function() {
    $.ajax({
			...
			type    : 'GET',
      data	  : { orderby: paramUrlSetup.orderby, column: paramUrlSetup.column, keyword: paramUrlSetup.keyword, branch_id: paramUrlSetup.branchId, category: 'Kandang' },
  ...
  $('.btn-download-template').click(function() {
		$.ajax({
			...
			headers : { 'Authorization': `Bearer ${token}` },
			type    : 'GET',
      data	  : { filename: 'Template Upload Barang Kandang.xlsx' },
			xhrFields: { responseType: 'blob' },
			beforeSend: function() { $('#loading-screen').show(); },
			success: function(data, status, xhr) {
				let disposition = xhr.getResponseHeader('content-disposition');
  ...
  $("#fileupload").fileupload({
    ...
    formData: { category: 'Kandang' },
		...
		$("#msg-box .modal-body").text('Berhasil Upload Kandang');
		$('#msg-box').modal('show');
		setTimeout(() => {
			$('#modal-upload-kandang').modal('toggle');
			loadKandang();
		}, 1000);
  ...
  $('#btnSubmitBarangKandang').click(function() {
    if (modalState == 'add') {
        ...
        fd.append('image', $(`#upload-image-1`)[0].files[0]);
			...
			$.ajax({
				...
				success: function(resp) {
					...
					$("#msg-box .modal-body").text('Berhasil Menambah Kandang');
					$('#msg-box').modal('show');

					setTimeout(() => {
						$('#modal-barang-kandang').modal('toggle');
						refreshForm(); loadKandang();
					}, 1000);
				}, complete: function() { $('#loading-screen').hide(); }
				, error: function(err) {
					if (err.status === 422) {
						let errText = ''; $('#beErr').empty(); $('#btnSubmitBarangKandang').attr('disabled', true);
    ...
    if (modalState == 'edit') {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#msg-box .modal-body").text('Berhasil Mengubah Kandang');
          $('#msg-box').modal('show');
  ...
  function loadKandang() {
    ...
    $.ajax({
        ...
        if(data.length) {
          $.each(data, function(idx, v) {
            ...
            + `<td>
                ${
                  ...
                  : `${(v.image)
                      ? `<a class="image-list-kandang" href="${v.image}"><img src="${v.image}" width="50" height="50"></a>`
          ...
          listKandang += `<tr class="text-center"><td colspan="11">Tidak ada data.</td></tr>`;
        }

				$('#list-kandang').append(listKandang);

				$('.openFormEdit').click(function() {
					const getObj = data.find(x => x.id == $(this).val());
					modalState = 'edit';

					$('.modal-title').text('Edit Barang Kandang');
        ...
        $('.image-list-kandang').magnificPopup({type:'image'});
  ...
  function validationForm() {
    ...
    validationBtnSubmitBarangKandang();
  }
  ...
  function formConfigure() {
    $('#selectedCabang').select2();
		...
		$('#modal-barang-kandang').modal('show');
  ...
  function validationBtnSubmitBarangKandang() {
    if (!isValidSelectedCabang || !isValidNamaBarang || !isValidJumlahBarang
      || !isValidHargaJual || !isValidHargaModal || !customErr1 || isBeErr) {
			...
			$('#btnSubmitBarangKandang').attr('disabled', false);
		}
  }
