$(document).ready(function() {
  ...
  $('.openFormAdd').click(function() {
		modalState = 'add';
    $('.modal-title').text('Tambah Barang Vitamin');
  ...
  $('.openFormUpload').click(function() {
		$('#modal-upload-vitamin .modal-title').text('Upload Barang Vitamin Sekaligus');
		$('#modal-upload-vitamin').modal('show');
		$('.validate-error').html('');
	});
  ...
  $('.btn-download-template').click(function() {
		$.ajax({
			...
			headers : { 'Authorization': `Bearer ${token}` },
			type    : 'GET',
      data	  : { filename: 'Template Upload Barang Vitamin.xlsx' },
			xhrFields: { responseType: 'blob' },
			beforeSend: function() { $('#loading-screen').show(); },
			success: function(data, status, xhr) {
				let disposition = xhr.getResponseHeader('content-disposition');
  ...
  $("#fileupload").fileupload({
    ...
    formData: { category: 'vitamin' },
		...
		$("#msg-box .modal-body").text('Berhasil Upload Vitamin');
		$('#msg-box').modal('show');
		setTimeout(() => {
			$('#modal-upload-vitamin').modal('toggle');
			loadVitamin();
		}, 1000);
  ...
  $('#btnSubmitBarangVitamin').click(function() {
    if (modalState == 'add') {
        ...
        fd.append('image', $(`#upload-image-1`)[0].files[0]);
			...
			$.ajax({
				...
				success: function(resp) {
					...
					$("#msg-box .modal-body").text('Berhasil Menambah Vitamin');
					$('#msg-box').modal('show');

					setTimeout(() => {
						$('#modal-barang-vitamin').modal('toggle');
						refreshForm(); loadVitamin();
					}, 1000);
				}, complete: function() { $('#loading-screen').hide(); }
				, error: function(err) {
					if (err.status === 422) {
						let errText = ''; $('#beErr').empty(); $('#btnSubmitBarangVitamin').attr('disabled', true);
    ...
    if (modalState == 'edit') {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#msg-box .modal-body").text('Berhasil Mengubah Vitamin');
          $('#msg-box').modal('show');
  ...
  function loadVitamin() {
    ...
    $.ajax({
        ...
        if(data.length) {
          $.each(data, function(idx, v) {
            listVitamin += `<tr>`
              ...
              + `<td>
                  ${(v.image) ? `<a class="image-list-vitamin" href="${v.image}"><img src="${v.image}" width="50" height="50"></a>` 
          ...
          listVitamin += `<tr class="text-center"><td colspan="11">Tidak ada data.</td></tr>`;
        }

				$('#list-vitamin').append(listVitamin);

				$('.openFormEdit').click(function() {
					const getObj = data.find(x => x.id == $(this).val());
					modalState = 'edit';

					$('.modal-title').text('Edit Barang Vitamin');
        ...
        $('.image-list-vitamin').magnificPopup({type:'image'});
  ...
  function formConfigure() {
    $('#selectedCabang').select2();
		...
		$('#modal-barang-vitamin').modal('show');
		$('#btnSubmitBarangVitamin').attr('disabled', true);
  }
  ...
  function validationBtnSubmitBarangVitamin() {
    if (!isValidSelectedCabang || !isValidNamaBarang || !isValidJumlahBarang
      || !isValidHargaJual || !isValidHargaModal || !customErr1 || isBeErr) {
			...
			$('#btnSubmitBarangVitamin').attr('disabled', false);
		}
  }
