$(document).ready(function() {
  ...
  $('.openFormAdd').click(function() {
		modalState = 'add';
    $('.modal-title').text('Tambah Barang Aksesoris');
  ...
  $('.openFormUpload').click(function() {
		$('#modal-upload-aksesoris .modal-title').text('Upload Barang Aksesoris Sekaligus');
		$('#modal-upload-aksesoris').modal('show');
		$('.validate-error').html('');
	});
  ...
  $('.downloadExcel').click(function() {
    $.ajax({
			...
			type    : 'GET',
      data	  : { orderby: paramUrlSetup.orderby, column: paramUrlSetup.column, keyword: paramUrlSetup.keyword, branch_id: paramUrlSetup.branchId, category: 'Aksesoris' },
  ...
  $('.btn-download-template').click(function() {
		$.ajax({
			...
			headers : { 'Authorization': `Bearer ${token}` },
			type    : 'GET',
      data	  : { filename: 'Template Upload Barang Aksesoris.xlsx' },
			xhrFields: { responseType: 'blob' },
			beforeSend: function() { $('#loading-screen').show(); },
			success: function(data, status, xhr) {
				let disposition = xhr.getResponseHeader('content-disposition');
  ...
  $("#fileupload").fileupload({
    ...
    formData: { category: 'Aksesoris' },
		...
		$("#msg-box .modal-body").text('Berhasil Upload Aksesoris');
		$('#msg-box').modal('show');
		setTimeout(() => {
			$('#modal-upload-aksesoris').modal('toggle');
			loadAksesoris();
		}, 1000);
  ...
  $('#btnSubmitBarangAksesoris').click(function() {
    if (modalState == 'add') {
        ...
        fd.append('image', $(`#upload-image-1`)[0].files[0]);
			...
			$.ajax({
				...
				success: function(resp) {
					...
					$("#msg-box .modal-body").text('Berhasil Menambah Aksesoris');
					$('#msg-box').modal('show');

					setTimeout(() => {
						$('#modal-barang-aksesoris').modal('toggle');
						refreshForm(); loadAksesoris();
					}, 1000);
				}, complete: function() { $('#loading-screen').hide(); }
				, error: function(err) {
					if (err.status === 422) {
						let errText = ''; $('#beErr').empty(); $('#btnSubmitBarangAksesoris').attr('disabled', true);
    ...
    if (modalState == 'edit') {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#msg-box .modal-body").text('Berhasil Mengubah Aksesoris');
          $('#msg-box').modal('show');
  ...
  function loadAksesoris() {
    ...
    $.ajax({
        ...
        if(data.length) {
          $.each(data, function(idx, v) {
            ...
            + `<td>
                ${
                  ...
                  : `${(v.image)
                      ? `<a class="image-list-aksesoris" href="${v.image}"><img src="${v.image}" width="50" height="50"></a>`
          ...
          listAksesoris += `<tr class="text-center"><td colspan="11">Tidak ada data.</td></tr>`;
        }

				$('#list-aksesoris').append(listAksesoris);

				$('.openFormEdit').click(function() {
					const getObj = data.find(x => x.id == $(this).val());
					modalState = 'edit';

					$('.modal-title').text('Edit Barang Aksesoris');
        ...
        $('.image-list-aksesoris').magnificPopup({type:'image'});
  ...
  function validationForm() {
    ...
    validationBtnSubmitBarangAksesoris();
  }
  ...
  function formConfigure() {
    $('#selectedCabang').select2();
		...
		$('#modal-barang-aksesoris').modal('show');
		$('#btnSubmitBarangAksesoris').attr('disabled', true);
  }
