$(document).ready(function() {
  ...
  $('.openFormAdd').click(function() {
		modalState = 'add';
    $('.modal-title').text('Tambah Barang Lain-lain');
  ...
  $('.openFormUpload').click(function() {
		$('#modal-upload-lain-lain .modal-title').text('Upload Barang Lain-lain Sekaligus');
		$('#modal-upload-lain-lain').modal('show');
		$('.validate-error').html('');
	});
  ...
  $('.downloadExcel').click(function() {
    $.ajax({
			...
			type    : 'GET',
      data	  : { orderby: paramUrlSetup.orderby, column: paramUrlSetup.column, keyword: paramUrlSetup.keyword, branch_id: paramUrlSetup.branchId, category: 'others' },
  ...
  $('.btn-download-template').click(function() {
		$.ajax({
			...
			headers : { 'Authorization': `Bearer ${token}` },
			type    : 'GET',
      data	  : { filename: 'Template Upload Barang Lain-lain.xlsx' },
			xhrFields: { responseType: 'blob' },
			beforeSend: function() { $('#loading-screen').show(); },
			success: function(data, status, xhr) {
				let disposition = xhr.getResponseHeader('content-disposition');
  ...
  $("#fileupload").fileupload({
    ...
    formData: { category: 'others' },
		...
		$("#msg-box .modal-body").text('Berhasil Upload Lain-lain');
		$('#msg-box').modal('show');
		setTimeout(() => {
			$('#modal-upload-lain-lain').modal('toggle');
			loadLainlain();
		}, 1000);
  ...
  $('#btnSubmitBarangLainlain').click(function() {
    if (modalState == 'add') {
        ...
        fd.append('image', $(`#upload-image-1`)[0].files[0]);
			...
			$.ajax({
				...
				success: function(resp) {
					...
					$("#msg-box .modal-body").text('Berhasil Menambah Lain-lain');
					$('#msg-box').modal('show');

					setTimeout(() => {
						$('#modal-barang-lain-lain').modal('toggle');
						refreshForm(); loadLainlain();
					}, 1000);
				}, complete: function() { $('#loading-screen').hide(); }
				, error: function(err) {
					if (err.status === 422) {
						let errText = ''; $('#beErr').empty(); $('#btnSubmitBarangLain-lain').attr('disabled', true);
    ...
    if (modalState == 'edit') {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#msg-box .modal-body").text('Berhasil Mengubah Lain-lain');
          $('#msg-box').modal('show');
  ...
  function loadLainlain() {
    ...
    $.ajax({
        ...
        if(data.length) {
          $.each(data, function(idx, v) {
            listLainlain += `<tr>`
              ...
              + `<td>
                  ${(v.image) ? `<a class="image-list-lain-lain" href="${v.image}"><img src="${v.image}" width="50" height="50"></a>` 
          ...
          listLainlain += `<tr class="text-center"><td colspan="11">Tidak ada data.</td></tr>`;
        }

				$('#list-lain-lain').append(listLainlain);

				$('.openFormEdit').click(function() {
					const getObj = data.find(x => x.id == $(this).val());
					modalState = 'edit';

					$('.modal-title').text('Edit Barang Lain-lain');
        ...
        $('.image-list-lain-lain').magnificPopup({type:'image'});
  ...
  function validationForm() {
    ...
    validationBtnSubmitBarangLainlain();
  }
  ...
  function formConfigure() {
    $('#selectedCabang').select2();
		...
		$('#modal-barang-lain-lain').modal('show');
		$('#btnSubmitBarangLainlain').attr('disabled', true);
  }
