$(document).ready(function() {
  ...
  $('.openFormAdd').click(function() {
		modalState = 'add';
    $('.modal-title').text('Tambah Barang Animal Food');
  ...
  $('.openFormUpload').click(function() {
		$('#modal-upload-animal-food .modal-title').text('Upload Barang Animal Food Sekaligus');
		$('#modal-upload-animal-food').modal('show');
		$('.validate-error').html('');
	});
  ...
  $('.downloadExcel').click(function() {
    $.ajax({
			...
			type    : 'GET',
      data	  : { orderby: paramUrlSetup.orderby, column: paramUrlSetup.column, keyword: paramUrlSetup.keyword, branch_id: paramUrlSetup.branchId, category: 'animal_food' },
  ...
  $('.btn-download-template').click(function() {
		$.ajax({
			...
			headers : { 'Authorization': `Bearer ${token}` },
			type    : 'GET',
      data	  : { filename: 'Template Upload Barang Animal Food.xlsx' },
			xhrFields: { responseType: 'blob' },
			beforeSend: function() { $('#loading-screen').show(); },
			success: function(data, status, xhr) {
				let disposition = xhr.getResponseHeader('content-disposition');
  ...
  $("#fileupload").fileupload({
    ...
    formData: { category: 'animal_food' },
		...
		$("#msg-box .modal-body").text('Berhasil Upload Animal Food');
		$('#msg-box').modal('show');
		setTimeout(() => {
			$('#modal-upload-animal-food').modal('toggle');
			loadAnimalFood();
		}, 1000);
  ...
  $('#btnSubmitBarangAnimalFood').click(function() {
    if (modalState == 'add') {
      ...
      fd.append('category', 'animal_food');
        ...
        fd.append('image', $(`#upload-image-1`)[0].files[0]);
			...
			$.ajax({
				...
				success: function(resp) {
					...
					$("#msg-box .modal-body").text('Berhasil Menambah Animal Food');
					$('#msg-box').modal('show');

					setTimeout(() => {
						$('#modal-barang-animal-food').modal('toggle');
						refreshForm(); loadAnimalFood();
					}, 1000);
				}, complete: function() { $('#loading-screen').hide(); }
				, error: function(err) {
					if (err.status === 422) {
						let errText = ''; $('#beErr').empty(); $('#btnSubmitBarangAnimalFood').attr('disabled', true);
    ...
    if (modalState == 'edit') {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#msg-box .modal-body").text('Berhasil Mengubah Animal Food');
          $('#msg-box').modal('show');
  ...
  function loadAnimalFood() {
    ...
    $.ajax({
        ...
        if(data.length) {
          $.each(data, function(idx, v) {
            listAnimalFood += `<tr>`
              ...
              + `<td>
                  ${(v.image) ? `<a class="image-list-animal-food" href="${v.image}"><img src="${v.image}" width="50" height="50"></a>` 
          ...
          listAnimalFood += `<tr class="text-center"><td colspan="11">Tidak ada data.</td></tr>`;
        }

				$('#list-animal-food').append(listAnimalFood);

				$('.openFormEdit').click(function() {
					const getObj = data.find(x => x.id == $(this).val());
					modalState = 'edit';

					$('.modal-title').text('Edit Barang Animal Food');
        ...
        $('.image-list-animal-food').magnificPopup({type:'image'});
  ...
  function validationForm() {
    ...
    validationBtnSubmitBarangAnimalFood();
  }
  ...
  function formConfigure() {
    $('#selectedCabang').select2();
		...
		$('#modal-barang-animal-food').modal('show');
		$('#btnSubmitBarangAnimalFood').attr('disabled', true);
  }
