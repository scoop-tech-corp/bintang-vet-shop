$(document).ready(function() {
	...
	function onSearch(keyword) {
		...
		$.ajax({
			url : $('.baseUrl').val() + '/api/daftar-barang/search',
			type: 'POST',
			dataType: 'JSON',
	...
	$('.onOrdering').click(function() {
		...
		loadDaftarBarang({orderBy: $(this).attr('orderby'), column: column});
	});
	...
	function loadDaftarBarang(param = null) {
		const orderBy = param ? param.orderBy : '';
		const column = param ? param.column : '';

		$.ajax({
			url     : $('.baseUrl').val() + '/api/daftar-barang',
			headers : { 'Authorization': `Bearer ${token}` },
			type    : 'GET',
			data	  : { orderby: orderBy, column: column },
			beforeSend: function() { $('#loading-screen').show(); },
			success: function(data) {
				let getId = null;
				let listDaftarBarang = '';
