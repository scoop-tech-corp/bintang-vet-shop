$(document).ready(function() {
  ...
  $('.openFormAdd').click(function() {
		modalState = 'add';
    $('.modal-title').text('Tambah Barang Pet Care');
  ...
  $('.openFormUpload').click(function() {
		$('#modal-upload-pet-care .modal-title').text('Upload Barang Pet Care Sekaligus');
		$('#modal-upload-pet-care').modal('show');
		$('.validate-error').html('');
	});
  ...
  $('.downloadExcel').click(function() {
    $.ajax({
			...
			type    : 'GET',
      data	  : { orderby: paramUrlSetup.orderby, column: paramUrlSetup.column, keyword: paramUrlSetup.keyword, branch_id: paramUrlSetup.branchId, category: 'pet_care' },
  ...
  $('.btn-download-template').click(function() {
		$.ajax({
			...
			headers : { 'Authorization': `Bearer ${token}` },
			type    : 'GET',
      data	  : { filename: 'Template Upload Barang Pet Care.xlsx' },
			xhrFields: { responseType: 'blob' },
			beforeSend: function() { $('#loading-screen').show(); },
			success: function(data, status, xhr) {
				let disposition = xhr.getResponseHeader('content-disposition');
  ...
  $("#fileupload").fileupload({
    ...
    formData: { category: 'pet_care' },
		...
		$("#msg-box .modal-body").text('Berhasil Upload Pet Care');
		$('#msg-box').modal('show');
		setTimeout(() => {
			$('#modal-upload-pet-care').modal('toggle');
			loadPetCare();
		}, 1000);
  ...
  $('#btnSubmitBarangPetCare').click(function() {
    if (modalState == 'add') {
        ...
        fd.append('image', $(`#upload-image-1`)[0].files[0]);
			...
			$.ajax({
				...
				success: function(resp) {
					...
					$("#msg-box .modal-body").text('Berhasil Menambah Pet Care');
					$('#msg-box').modal('show');

					setTimeout(() => {
						$('#modal-barang-pet-care').modal('toggle');
						refreshForm(); loadPetCare();
					}, 1000);
				}, complete: function() { $('#loading-screen').hide(); }
				, error: function(err) {
					if (err.status === 422) {
						let errText = ''; $('#beErr').empty(); $('#btnSubmitBarangPetCare').attr('disabled', true);
    ...
    if (modalState == 'edit') {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#msg-box .modal-body").text('Berhasil Mengubah Pet Care');
          $('#msg-box').modal('show');
  ...
  function loadPetCare() {
    ...
    $.ajax({
        ...
        if(data.length) {
          $.each(data, function(idx, v) {
            listPetCare += `<tr>`
              ...
              + `<td>
                  ${(v.image) ? `<a class="image-list-pet-care" href="${v.image}"><img src="${v.image}" width="50" height="50"></a>` 
          ...
          listPetCare += `<tr class="text-center"><td colspan="11">Tidak ada data.</td></tr>`;
        }

				$('#list-pet-care').append(listPetCare);

				$('.openFormEdit').click(function() {
					const getObj = data.find(x => x.id == $(this).val());
					modalState = 'edit';

					$('.modal-title').text('Edit Barang Pet Care');
        ...
        $('.image-list-pet-care').magnificPopup({type:'image'});
  ...
  function validationForm() {
    ...
    validationBtnSubmitBarangPetCare();
  }
  ...
  function formConfigure() {
    $('#selectedCabang').select2();
		...
		$('#modal-barang-pet-care').modal('show');
		$('#btnSubmitBarangPetCare').attr('disabled', true);
  }
