$(document).ready(function() {
  ...
  $('.openFormAdd').click(function() {
		modalState = 'add';
    $('.modal-title').text('Tambah Barang Dog Food');
  ...
  $('.openFormUpload').click(function() {
		$('#modal-upload-dog-food .modal-title').text('Upload Barang Dog Food Sekaligus');
		$('#modal-upload-dog-food').modal('show');
		$('.validate-error').html('');
	});
  ...
  $('.downloadExcel').click(function() {
    $.ajax({
			...
			type    : 'GET',
      data	  : { orderby: paramUrlSetup.orderby, column: paramUrlSetup.column, keyword: paramUrlSetup.keyword, branch_id: paramUrlSetup.branchId, category: 'dog_food' },
  ...
  $('.btn-download-template').click(function() {
		$.ajax({
			...
			headers : { 'Authorization': `Bearer ${token}` },
			type    : 'GET',
      data	  : { filename: 'Template Upload Barang Dog Food.xlsx' },
			xhrFields: { responseType: 'blob' },
			beforeSend: function() { $('#loading-screen').show(); },
			success: function(data, status, xhr) {
				let disposition = xhr.getResponseHeader('content-disposition');
  ...
  $("#fileupload").fileupload({
    ...
    formData: { category: 'dog_food' },
		...
		$("#msg-box .modal-body").text('Berhasil Upload Dog Food');
		$('#msg-box').modal('show');
		setTimeout(() => {
			$('#modal-upload-dog-food').modal('toggle');
			loadDogFood();
		}, 1000);
  ...
  $('#btnSubmitBarangDogFood').click(function() {
    if (modalState == 'add') {
        ...
        fd.append('image', $(`#upload-image-1`)[0].files[0]);
			...
			$.ajax({
				...
				success: function(resp) {
					...
					$("#msg-box .modal-body").text('Berhasil Menambah Dog Food');
					$('#msg-box').modal('show');

					setTimeout(() => {
						$('#modal-barang-dog-food').modal('toggle');
						refreshForm(); loadDogFood();
					}, 1000);
				}, complete: function() { $('#loading-screen').hide(); }
				, error: function(err) {
					if (err.status === 422) {
						let errText = ''; $('#beErr').empty(); $('#btnSubmitBarangDogFood').attr('disabled', true);
    ...
    if (modalState == 'edit') {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          $("#msg-box .modal-body").text('Berhasil Mengubah Dog Food');
          $('#msg-box').modal('show');
  ...
  function loadDogFood() {
    ...
    $.ajax({
        ...
        if(data.length) {
          $.each(data, function(idx, v) {
            listDogFood += `<tr>`
              ...
              + `<td>
                  ${(v.image) ? `<a class="image-list-dog-food" href="${v.image}"><img src="${v.image}" width="50" height="50"></a>` 
          ...
          listDogFood += `<tr class="text-center"><td colspan="11">Tidak ada data.</td></tr>`;
        }

				$('#list-dog-food').append(listDogFood);

				$('.openFormEdit').click(function() {
					const getObj = data.find(x => x.id == $(this).val());
					modalState = 'edit';

					$('.modal-title').text('Edit Barang Dog Food');
        ...
        $('.image-list-dog-food').magnificPopup({type:'image'});
  ...
  function validationForm() {
    ...
    validationBtnSubmitBarangDogFood();
  }
  ...
  function formConfigure() {
    $('#selectedCabang').select2();
		...
		$('#modal-barang-dog-food').modal('show');
		$('#btnSubmitBarangDogFood').attr('disabled', true);
  }
