$(document).ready(function () {
  ...
  $(".btn-download-laporan").click(function () {
    $.ajax({
      url: $(".baseUrl").val() + "/api/daily-finance-report/generate",
      headers: { Authorization: `Bearer ${token}` },
      type: "GET",
      data: {
        orderby: paramUrlSetup.orderby,
        column: paramUrlSetup.column,
        date: paramUrlSetup.date,
  ...
  function loadLaporanKeuanganHarian() {
    $.ajax({
      url: $(".baseUrl").val() + "/api/daily-finance-report",
      headers: { Authorization: `Bearer ${token}` },
      type: "GET",
      data: {
        orderby: paramUrlSetup.orderby,
      ...
      success: function (resp) {
        ...
        if (getData.length) {
          ...
          listLaporanKeuanganHarian += `<tr class="text-center"><td colspan="10">Tidak ada data.</td></tr>`;
        }
