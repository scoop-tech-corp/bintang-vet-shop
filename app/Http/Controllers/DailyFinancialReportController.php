...
class DailyFinancialReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->user()->role == 'kasir') {
            $item = $item->where('mp.branch_id', '=', $request->user()->branch_id);
        }

        if ($request->date) {

            $item = $item->where(DB::raw('DATE(py.created_at)'), '=', $request->date);
        }
        ...
        if ($request->date) {
            ...
            $avg = $avg->where(DB::raw('DATE(py.created_at)'), '=', $request->date);
        }
